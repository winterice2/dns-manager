...
- `📊 Статистика` (`show_stats_tab`):
  - Общая статистика проекта (версия 1.1.0, количество провайдеров: 6).
  - Краткая информация о каждом DNS провайдере с ссылками.
  - Полезные ссылки на документацию и сайты провайдеров.

**Важно:** При добавлении новых функций или элементов UI, убедитесь, что они размещаются в соответствующей вкладке, чтобы сохранить чистоту и порядок интерфейса. Текущая структура позволяет легко расширять функционал без перегрузки одного экрана.
